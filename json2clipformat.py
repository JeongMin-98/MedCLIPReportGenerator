...
for item in tqdm(data, desc="Embedding"):
    ...
    try:
        ...
        if len(temp_batch) == BATCH_SIZE:
            # 배치 임베딩
            inputs = clip_processor(images=temp_batch, return_tensors="pt", padding=True).to(device)
            with torch.no_grad():
            ...
            for i, item in enumerate(temp_items):
                ...
                clip_embedding_dict[patient_id] = features[i]
                captions.append({
                    "image_id": patient_id,
                    "caption": diagnosis,
...
for cls, items in class_to_items.items():
    ...
    for item in train_items:
        train_embeddings[item["image_id"]] = clip_embedding_dict[item["image_id"]]
    for item in test_items:
        test_embeddings[item["image_id"]] = clip_embedding_dict[item["image_id"]]

# 학습용 데이터 저장
train_output = {
    "clip_embedding": train_embeddings,
    "captions": train_captions
}
with open("clipcap_dataset_train.pkl", "wb") as f:
    pickle.dump(train_output, f)

# 테스트용 데이터 저장
test_output = {
    "clip_embedding": test_embeddings,
    "captions": test_captions
}
with open("clipcap_dataset_test.pkl", "wb") as f:
    pickle.dump(test_output, f)

print("✅ 정제 및 저장 완료: train → clipcap_dataset_train.pkl, test → clipcap_dataset_test.pkl")
